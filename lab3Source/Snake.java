...
/**
 * This is what make the game tick. It updates the position according to the
 * controller.
 * 
...
public class Snake implements Runnable {
	/**
	 * Use this lock to avoid concurrent modification of the ArrayLists body and
	 * bonus.
	 */
	...
	public Snake(int startSize, int startX, int startY, int startDir) {
		...
		for (int i = 0; i < startSize; i++) {
			body.add(new Rectangle(startX + boxSize * i, startY, boxSize
					- borderSize, boxSize - borderSize));
		}
	}

	/**
	 * Overloads run() in the Runnable interface. Updates the position of the
	 * snake and check whether it is outside or takes a bonus. Also randomizes
	 * bonuses randomly in time.
	 */
	public void run() {
		while (true) {
			Snake.lock.lock();
			...
			for (int i = 0; i < body.size(); i++) {
				...
				if (r.x < 0 || r.y < 0 || r.x > GamePanel.getDimension().width
			...
			if (0 == (int) Math.round(Math.random() * 10)) {
				bonus.add(new Rectangle((int) Math.round(Math.random()
						* (400 - boxSize)), (int) Math.round(Math.random()
						* (400 - boxSize)), boxSize, boxSize));
			}
			Snake.lock.unlock();
			GamePanel.rePaint();
	...
	/**
	 * Uses the current graphics to draw the background, snake and bonuses on
	 * the screen
	 */
	public void paint() {
	...
	public void setKeyControll() {
		...
		if (this.controller == null) {
			ControllerKey controller = new ControllerKey();
			this.controller = controller;
		}
		this.controller.setDirection(dir);
	...
	/**
	 * Sets that the "3D" graphics are used. This method should be implemented
	 * during the labs
	 */
	public void set3dGraphics() {

		renderer = new Graphics3DAdapter(Color.GREEN, Color.RED);
		renderer.setSnakeColor(Color.blue);
		// YOUR CODE HERE
	...
	/**
	 * Sets that the package graphics are used. This method should be
	 * implemented during the labs
	 */
	...
	/**
	 * Sets that the graphics with shadows are used. This method should be
	 * implemented during the labs
	 */
